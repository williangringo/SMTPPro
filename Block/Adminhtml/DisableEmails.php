 ...
 * @author     WebFlakeStudio - updated by Kelvin
 */
class Aschroder_SMTPPro_Block_Adminhtml_DisableEmails
    extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    ...
    protected function _getValues()
    {
        ...
        if (empty($this->_values)) {           
            $this->_values = array(
                array('label' => Mage::helper('smtppro')->__('On'), 'value' => 0),
                array('label' => Mage::helper('smtppro')->__('Off'), 'value' => 1)
            );
        }
        return $this->_values;
    }
    ...
    protected function _getFieldHtml($fieldset, $notificationType)
    {
       $configData = $this->getConfigData();
        $path = Aschroder_SMTPPro_Model_Email_Template::XML_PATH_PREFIX . $notificationType['value'];
        ...
        $field = $fieldset->addField('notify_' . $notificationType['value'], 'select',
            array(
                'name'          => 'groups[aschroder_smtppro][fields][' . $notificationType['value'] . '][value]',
